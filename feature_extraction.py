...
parser.add_argument('--pool', '-p', metavar='NAME', default='avg',
                    help='')
parser.add_argument('--feature_dim', default=1024, type=int, help='')
parser.add_argument('--feature_outdir', default="beit_512", type=str, help='')
...
def validate(args):
    ...
    if args.test_pool:  # 该test_pool不能用于特征处理
    ...
    # --------------
    gallery_tensor = torch.zeros(len(dataset_gallery), args.feature_dim)
    query_tensor = torch.zeros(len(dataset_query), args.feature_dim)
    similarity_tensor = torch.zeros(len(dataset_query), len(dataset_gallery))
    global query_tensor_list,gallery_tensor_list,similarity_tensor_list
    query_tensor_list = []
    gallery_tensor_list = []
    similarity_tensor_list = []
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in tqdm(enumerate(loader_query)):
            ...
            with amp_autocast():
                # -------------
                # output = model(input) # model.forward_features(input)
                if args.num_gpu > 1:
                    output = model.module.forward_features(input)
                else:
                    output = model.forward_features(input)
                if 'beit' in args.model:
                    output = output[:, 1:].mean(dim=1)  # output = Pooling(output).squeeze()
                output = output/(torch.norm(output,p=2,dim=1).unsqueeze(1))
                query_tensor[batch_idx*args.batch_size:batch_idx*args.batch_size+output.size(0)] = output
        ...
        for batch_idx, (input, target) in tqdm(enumerate(loader_gallery)):
            if args.no_prefetcher:
                target = target.to(device)
                input = input.to(device)
            if args.channels_last:
            ...
            with amp_autocast():
                ...
                output = output/(torch.norm(output,p=2,dim=1).unsqueeze(1))
                gallery_tensor[batch_idx*args.batch_size:batch_idx*args.batch_size+output.size(0)] = output

                temp_tensor = torch.mm(query_tensor.to(device), torch.transpose(output,0,1).float())
                similarity_tensor[:,batch_idx*args.batch_size:batch_idx*args.batch_size+output.size(0)] = temp_tensor
    ...
    return query_tensor, gallery_tensor, similarity_tensor, dataset_query.reader, dataset_gallery.reader
...
def main():
    ...
    if len(model_cfgs):
        ...
        if args.retry:
            ...
            # results = validate(args)
            query_tensor, gallery_tensor, similarity_tensor, query_names, gallery_names = validate(args)
            query_names = [item[0].name.split("/")[-1] for item in query_names]
            gallery_names = [item[0].name.split("/")[-1].split('.')[0] for item in gallery_names]
            retreival_results = []

            similarity_index = similarity_tensor.topk(k=20, dim=1)[1]  # indices
