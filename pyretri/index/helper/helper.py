...
class IndexHelper:
    ...
    def do_index(self, query_fea: np.ndarray, query_info: List, gallery_fea: np.ndarray) -> (List, np.ndarray, np.ndarray):
        ...
        sorted_index, dis, query_fea_qe  = self.re_rank(query_fea, gallery_fea, dis=dis, sorted_index=sorted_index)
        # 经过qe之后，query的feature发生了变化，因此也输出一下 
        ...
        del sorted_index
        # return dis
        return query_fea, query_fea_qe, gallery_fea, dis
