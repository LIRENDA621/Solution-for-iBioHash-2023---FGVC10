...
class KNN(MetricBase):
    ...
    def _cal_dis(self, query_fea: torch.tensor, gallery_fea: torch.tensor) -> torch.tensor:
        ...
        #     dis = dis.transpose(1, 0)
        #     return dis
        # else:  # for dba and all dis.  1. dba not use dis,so dis can partition 
        #     sorted_index = torch.zeros(gallery_fea.size(0), self._hyper_params["top_k"])  # error :torch.zeros(query_fea.size(0), self._hyper_params["top_k"])
        #     query_fea = query_fea.transpose(1, 0)
            
        #     for i in tqdm(range(math.ceil(gallery_fea.size(0)/400))):
        #         # dis=torch.zeros((query_fea.size(0),400))
        #         dis = gallery_fea[i*400: min((i+1)*400,gallery_fea.size(0))].mm(query_fea)
        #         # dis = dis.transpose(1, 0)
        #         _, sorted_index[i*400:min((i+1)*400,gallery_fea.size(0))] = dis.topk(self._hyper_params["top_k"], 1)
    ...
    def __call__(self, query_fea: torch.tensor, gallery_fea: torch.tensor) -> (torch.tensor, torch.tensor):
        ...
        if self._hyper_params["top_k"] == 0:
            ...
            _, sorted_index = dis.topk(self._hyper_params["top_k"], 1)
            print("size of sorted_index is {}".format(sorted_index.size()))
        return dis, sorted_index
        ...
        #     print("size of sorted_index is {}".format(sorted_index.size()))
        #     return dis, sorted_index
        # elif (query_fea.size(0)<5000) and (self._hyper_params["top_k"] != 0):
        #     dis = self._cal_dis(query_fea, gallery_fea)
        #     _, sorted_index = dis.topk(self._hyper_params["top_k"], 1)
        #     # sorted_index = sorted_index[:, :self._hyper_params["top_k"]]
        #     return dis, sorted_index
        # else:
        #     dis, sorted_index = self._cal_dis(query_fea, gallery_fea)
        #     # _, sorted_index = dis.topk(self._hyper_params["top_k"], 1)
        #     # sorted_index = sorted_index[:, :self._hyper_params["top_k"]]
        #     return dis, sorted_index