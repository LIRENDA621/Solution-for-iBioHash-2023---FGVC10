...
class KReciprocal(ReRankerBase):
    ...
    def __call__(self, query_fea: torch.tensor, gallery_fea: torch.tensor,  dis: torch.tensor or None = None,
        ...
        q_g_dist = dis.cpu()
        g_g_dist = self._cal_dis(gallery_fea, gallery_fea).cpu()
        q_q_dist = self._cal_dis(query_fea, query_fea).cpu()
        
        original_dist = torch.concat(
            [torch.concat([q_q_dist, q_g_dist], axis=1),
             torch.concat([q_g_dist.T, g_g_dist], axis=1)],
            axis=0)
        original_dist = 40-2*original_dist
        ...
        for i in range(all_num):
            ...
            for j in range(len(k_reciprocal_index)):
                ...
                candidate_forward_k_neigh_index = initial_rank[candidate, :int(math.ceil(self._hyper_params["k1"] / 2.)) + 1]
                candidate_backward_k_neigh_index = initial_rank[candidate_forward_k_neigh_index,
                                                   :int(math.ceil(self._hyper_params["k1"] / 2.)) + 1]
        ...
        if torch.cuda.is_available():
            ...
            sorted_idx = torch.argsort(final_dist, dim=1, descending=True)
        else:
