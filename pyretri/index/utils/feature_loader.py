...
class PickleLoader:
    ...
    def load(self, fea_dir: str) -> (np.ndarray, np.ndarray, List, List):
        
        assert os.path.exists(fea_dir), "non-exist feature path: {}".format(fea_dir)

        print("[LoadFeature]: loading feature from {}...".format(fea_dir))
        with open(os.path.join(fea_dir,'query.pkl'), "rb") as f:
            query = pickle.load(f)

        with open(os.path.join(fea_dir,'gallery.pkl'), "rb") as f:
            gallery = pickle.load(f)
        
        with open(os.path.join(fea_dir,'names.pkl'), "rb") as f:
            data = pickle.load(f)
            query_names = [item[0].name.split("/")[-1] for item in data['query']]
            gallery_names = [item[0].name.split("/")[-1].split('.')[0] for item in data['gallery']]
            # query_names = [{'name':item[0].name.split("/")[-1].split('.')[0]} for item in data['query']]
            # gallery_names = [{'name':item[0].name.split("/")[-1]}.split('.')[0] for item in data['gallery']]
            
        return query, gallery, query_names, gallery_names


# feature_loader = FeatureLoader()

feature_loader = PickleLoader()
